...
{
	std::string thisversion = TONOKA_VERSION;
	std::string version, mustupdate, message;
	tonoka_updateCheck updatecheckdlg(NULL);
	...
	try
	{
		...
		version = pt.get_child("tonoka.version").data();
		mustupdate = pt.get_child("tonoka.mustupdate").data();
		message = pt.get_child("tonoka.message").data();
	...
	if (version != thisversion)
	{
		...
		updatecheckdlg.m_updatetext = wxT("A new version of tonoka is available:");
		updatecheckdlg.ShowModal();
	}
...
int getUpdateJSON(std::string &updateinfo)
{
	...
	try
	{
		...
		std::string checkurl = "/software/tonoka/";
