...
window.Wodo = window.Wodo || (function () {
    ...
    function TextEditor(mainContainerElementId, editorOptions) {
        ...
        */
        ...
        runtime.assert(Boolean(mainContainerElement),
            "No id of an existing element passed to Wodo.createTextEditor(): " + mainContainerElementId);
        ...
        function createSession() {
            ...
            if (undoRedoEnabled) {
                ...
                editorSession.sessionController.getUndoManager().subscribe(
                    gui.UndoManager.signalDocumentModifiedChanged, relayModifiedSignal);
            }

            // Relay any metadata changes to the Editor's consumer as an event
            editorSession.sessionController.getMetadataController().subscribe(
                gui.MetadataController.signalMetadataChanged, relayMetadataSignal);
        ...
        this.closeDocument = function (callback) {
            ...
            session.close(function (err) {
                if (err) {
                    ...
                    editorSession.sessionController.getMetadataController().unsubscribe(
                        gui.MetadataController.signalMetadataChanged, relayMetadataSignal);
