...
// #include "PThread.h"
// 
// class PSearchDicomthread:public PThread
// {
// public:
//     PSearchDicomthread (const char * ThreadName = "PSearchDicomthread",OFString dir)
//     {
//         PThread(ThreadName);
//         m_SearchDir = dir;
//         m_direxist = OFTrue;
//         if (!OFStandard::dirExists(m_SearchDir))
//         {
//             OFLOG_ERROR(storescuLogger,m_SearchDir+"->Dir not exits");
//             m_direxist = OFFalse;
//         }
//     }
//     ~PSearchDicomthread(void)
//     {
// 
//     }
//     virtual void Run()
...
#endif
        {
            OFLOG_ERROR(storescuLogger,"mkdir :"+dir+"  .error!");
            return OFFalse;
        }
        else
        {
...
int FindAndSendImage(int argc, char *argv[]);
void FindAndSendThread(void* param)
{
    int argc;
    char **argv;
    argc = ((SendParm*)param)->argc;
    argv = ((SendParm*)param)->argv;
...
int main(int argc, char *argv[])
{
    ...
    _beginthread(FindAndSendThread, 0, (void *)&param);
    while(!thread_end)
    {
        OFStandard::sleep(1);
    }
...
int FindAndSendImage(int argc, char *argv[])
{
    //OFString file_send_name = "C:\\100dimage\\add123.txt";
    //readSendFile(file_send_name);
    //searchdicomfile("\\\\192.168.0.124\\common\\Test_dcmtk_rec\\Images\\0");
    //192.168.0.158 1042 --required --propose-lossless \\192.168.0.124\common\Test_dcmtk_rec\Images\
    //--------------------增加日志文件的方式----------------------------------------------------------------
    ...
    OFString logfilename = log_dir +"/DcmStoreSCU.log";//"/home/zyq/code/C++/DicomScuApp/DicomSCU/bin/Debug/dcmtk_storescu";
    ...
    OFLOG_INFO(storescuLogger,"---------argv[]:"+tempstr+" ----------------------");
    OFLOG_INFO(storescuLogger,"---------<--DcmNet storescu start run!-->---------------");

    //设置部分默认值
#if HAVE_WINDOWS_H
    opt_networkTransferSyntax = EXS_JPEGProcess14SV1;  //增加
#endif
    //-----------------------------------------------------------------------------------------------------
    ...
    if (app.parseCommandLine(cmd, argc, argv))
    {
        ...
        if (cmd.findOption("--propose-jpeg8"))
            opt_networkTransferSyntax = EXS_JPEGProcess1;
        ...
        if (cmd.findOption("--required"))
            opt_proposeOnlyRequiredPresentationContexts = OFTrue;
        if (cmd.findOption("--combine"))
        ...
        if (cmd.findOption("--uid-padding"))
            opt_correctUIDPadding = OFTrue;

        if (cmd.findOption("--invent-instance"))
        ...
        if (cmd.findOption("--verify-peer-cert"))
    ...
    for (int i = 3; i <= paramCount; i++)
    {
        ...
        if (OFStandard::dirExists(paramString))
        {
            if (opt_scanDir)
            {
                OFLOG_INFO(storescuLogger,"------------start searchDirectory and Recursive: "+OFString(paramString));
                //OFStandard::searchDirectoryRecursively(paramString, inputFiles, opt_scanPattern, "" /*dirPrefix*/, opt_recurse);
                //修改为遍历目录同时文件索引为ini记录

                getSendFile(paramString, inputFiles);
                dicom_total_count = inputFiles.size();

                OFLOG_INFO(storescuLogger,"------------search Directory and Recursive end! : "+OFString(paramString));
            }
            else
            {
    ...
    while (if_iter != if_last)
    {
        ...
        OFString filename = currentFilename;
        //if (OFStandard::fileExists(currentFilename))
        OFString temS = filename.substr(filename.length()-4,4);
        char suffix[5];
        strcpy(suffix,temS.c_str());
        for (int i=1; i<4; i++)
        {
            if (suffix[i] <='z' && suffix[i]>='a')
            {
                suffix[i] = suffix[i]-32;
            }
        }
        OFString ofstr(suffix);
       if ((ofstr == ".DCM" || suffix == ".DIC" ) /*&& OFStandard::fileExists(currentFilename)*/)
       {
        ...
        {
            errormsg = "cannot access file or not .dcm file: ";
            errormsg += currentFilename;
            if (opt_haltOnUnsuccessfulStore)
            {
                OFLOG_FATAL(storescuLogger, errormsg);
    ...
    dicom_send_count = 0;
    OFLOG_INFO(storescuLogger,"start send dicom, total count:"<<dicom_send_count);
    while ((iter != enditer) && cond.good())
    {
        cond = cstore(assoc, *iter);
        ++iter;
        dicom_send_count++;
        if (dicom_send_count%20 == 0)
        {
            OFLOG_INFO(storescuLogger,"send dicom/dicom total count:"<<dicom_send_count<<"/"<<dicom_total_count);
            COUT<<"send dicom/dicom total count:"<<dicom_send_count<<"/"<<dicom_total_count<<OFendl;
        }
    }
    //cout<<dicom_total_count<<":send dicom finish!"<<endl;
    OFLOG_INFO(storescuLogger,""<<dicom_total_count<<"::send dicom finish!");
...
*/
{
    ...
    if (cond != EC_Normal)
    {
        OFString temp(cond.text());
        OFLOG_WARN(storescuLogger,temp + " ->send dicom fail!:"+fname);
    }
    if (!opt_haltOnUnsuccessfulStore)
    {
        cond = EC_Normal;
    ...
    {
        OFLOG_INFO(storescuLogger,"send dicom ok!:"+fname);
    }
